...
Background::Background() {
    for (int i = 0; i < 120; ++i)
        m_stars.push_back({ {rnd(0,Cfg::W), rnd(0,Cfg::H)},
            rnd(10.f,20.f), rnd(60.f,130.f), rnd(0.8f,1.4f) });
  
    for (int i = 0; i < 70; ++i)
        m_stars.push_back({ {rnd(0,Cfg::W), rnd(0,Cfg::H)},
            rnd(22.f,45.f), rnd(140.f,200.f), rnd(1.2f,2.0f) });
    
    for (int i = 0; i < 30; ++i)
        m_stars.push_back({ {rnd(0,Cfg::W), rnd(0,Cfg::H)},
            rnd(50.f,90.f), rnd(200.f,255.f), rnd(1.6f,2.8f) });
...
void Background::rebuildVerts() {
    for (std::size_t i = 0; i < m_stars.size(); ++i) {
        ...
        m_starVertsCpu[vi + 0] = { tl,col }; m_starVertsCpu[vi + 1] = { tr,col };
        m_starVertsCpu[vi + 2] = { br,col }; m_starVertsCpu[vi + 3] = { tl,col };
        m_starVertsCpu[vi + 4] = { br,col }; m_starVertsCpu[vi + 5] = { bl,col };
    }
...
void Background::draw(sf::RenderTarget& rt) const {
    sf::RectangleShape bg({ float(Cfg::W), float(Cfg::H) });
    bg.setFillColor({ 4, 4, 18, 255 });
    rt.draw(bg);
    ...
    std::array<sf::Color, 3> nebCol = {
        sf::Color{40,10,80,22}, sf::Color{10,30,70,18}, sf::Color{60,10,40,16}
    };
    for (int i = 0; i < 3; ++i) {
        sf::CircleShape c(m_nebRadii[i]);
        c.setOrigin({ c.getRadius(), c.getRadius() });
        c.setPosition(nebPos[i]);
        c.setFillColor(nebCol[i]);
        rt.draw(c);
    }
