...
static int scoreFor(EnemyType t) {
    switch (t) {
    case EnemyType::Drone:    return Cfg::SCORE_DRONE;
    case EnemyType::Invader:  return Cfg::SCORE_INVADER;
    case EnemyType::Brute:    return Cfg::SCORE_BRUTE;
    case EnemyType::Kamikaze: return Cfg::SCORE_KAMIKAZE;
    case EnemyType::Elite:    return Cfg::SCORE_ELITE;
    default:                  return 0;
    }
}
static sf::Color colFor(EnemyType t) {
    switch (t) {
    case EnemyType::Drone:    return Cfg::COL_DRONE;
    case EnemyType::Invader:  return Cfg::COL_INVADER;
    case EnemyType::Brute:    return Cfg::COL_BRUTE;
    case EnemyType::Kamikaze: return Cfg::COL_KAMIKAZE;
    case EnemyType::Elite:    return Cfg::COL_ELITE;
    default:                  return Cfg::COL_UFO;
    }
}
static int hpFor(EnemyType t) {
    if (t == EnemyType::Brute) return 2;
    if (t == EnemyType::Elite) return 3;
    return 1;
}

static EnemyType typeForRow(int row) {
    switch (row) {
    case 0: return EnemyType::Elite;
    case 1: return EnemyType::Kamikaze;
    case 2: return EnemyType::Brute;
    case 3: return EnemyType::Drone;
    default:return EnemyType::Invader;
    }
}
...
void EnemyManager::spawnFormation(int wave) {
    ...
    for (int r = 0; r < Cfg::FORMATION_ROWS; ++r) {
        ...
        for (int c = 0; c < Cfg::FORMATION_COLS; ++c) {
            ...
            e.shootInterval = rnd(Cfg::ENEMY_SHOOT_MIN, Cfg::ENEMY_SHOOT_MAX)
...
PowerupType EnemyManager::randomDrop() {
    float r = rnd(0.f, 1.f);
    if (r > Cfg::POWERUP_DROP_CHANCE) return PowerupType::None;
    float t = rnd(0.f, 1.f);
    if (t < 0.33f) return PowerupType::Rapid;
    if (t < 0.66f) return PowerupType::Triple;
...
void EnemyManager::applyTypeMovement(Enemy& e, float dt, sf::Vector2f playerPos) {
    ...
    case EnemyType::Kamikaze:
        if (e.isDiving) {
            ...
            e.diveTimer -= dt;
            float diveChance = (Cfg::KAMIKAZE_DIVE_BASE
                + m_wave * Cfg::KAMIKAZE_DIVE_WAVE) * dt * 60.f;
...
void EnemyManager::tryEnemyShoot(Enemy& e, BulletManager& bullets, float dt) {
    e.shootTimer -= dt;
    if (e.shootTimer > 0.f) return;
    if (e.type == EnemyType::Kamikaze && e.isDiving) return;
    ...
    HitCallback onKill, KamikazeHitCallback onKamikazeHit)
{
    ...
    if (!m_stepping) {
        ...
        if (m_stepY >= Cfg::FORM_STEP_V) {
            float delta = m_stepY;
            for (auto& e : m_enemies) e.formationBase.y += delta;
            m_stepY = 0.f;
            m_stepping = false;
        }
    }
        ...
        if (!e.alive) {
            if (e.exploding) {
                ...
                if (e.explodeTimer <= 0.f) {
                    e.explodeTimer = 0.1f;
                    e.explodeFrame++;
                    if (e.explodeFrame >= 4) e.exploding = false;
                }
            }
            continue;
        }
        if (e.hitFlash > 0.f) e.hitFlash -= dt;
    ...
    for (auto& b : bullets.getAll()) {
        ...
        for (auto& e : m_enemies) {
            ...
            if (dx < 0.f) dx = -dx;
            if (dx > e.halfW) continue;

            float dy = b.pos.y - e.pos.y;
            if (dy < 0.f) dy = -dy;
            if (dy > e.halfH) continue;
    ...
    for (auto& e : m_enemies) {
        if (!e.alive || e.type != EnemyType::Kamikaze || !e.isDiving) continue;
        ...
        if (distSq > KAMIKAZE_HIT_RADIUS * KAMIKAZE_HIT_RADIUS) continue;
    ...
    for (auto& p : m_powerups) {
        ...
        if (p.pos.y > float(Cfg::H) + 30.f) p.alive = false;
    }
...
void EnemyManager::draw(sf::RenderTarget& rt) const {
    ...
    for (const auto& p : m_powerups) {
        ...
        const sf::Texture& tex =
            (p.type == PowerupType::Rapid) ? GFX.powerupRapid :
            (p.type == PowerupType::Triple) ? GFX.powerupTriple :
            GFX.powerupShield;
        sf::Sprite spr(tex);
        auto sz = tex.getSize();
    ...
    for (int t = 0; t < 5; ++t) {
        ...
        for (const auto& e : m_enemies) {
            ...
            if (static_cast<int>(e.type) != t) continue;
            ...
            if (e.maxHp > 1 && e.hp < e.maxHp) ++hpCount;
        }
        ...
        if (hpCount > 0) {
            ...
            for (const auto& e : m_enemies) {
                if (!e.alive || e.exploding) continue;
                if (static_cast<int>(e.type) != t) continue;
                if (e.maxHp <= 1 || e.hp >= e.maxHp) continue;
...
sf::Vector2f EnemyManager::getLowestEnemyPos() const {
    ...
    for (const auto& e : m_enemies)
        if (e.alive && e.pos.y > lowest.y) lowest = e.pos;
    return lowest;
}