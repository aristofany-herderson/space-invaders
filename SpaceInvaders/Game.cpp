        ...
        sf::Style::Titlebar | sf::Style::Close)
{
    ...
    m_font.openFromFile("assets/fonts/font.ttf");
...
void Game::processEvents() {
    while (const auto ev = m_window.pollEvent()) {
        if (ev->is<sf::Event::Closed>()) { m_window.close(); return; }

        if (const auto* kp = ev->getIf<sf::Event::KeyPressed>()) {
            switch (m_state) {
            case GameState::Menu:
                if (kp->code == sf::Keyboard::Key::Enter ||
                    kp->code == sf::Keyboard::Key::Space) startGame();
            ...
            case GameState::Win:
                if (kp->code == sf::Keyboard::Key::Enter ||
                    kp->code == sf::Keyboard::Key::Space) m_state = GameState::Menu;
...
void Game::update(float dt) {
    ...
    if (m_comboTimer > 0.f) m_comboTimer -= dt; else m_combo = 0;
    if (m_waveTextTimer > 0.f) m_waveTextTimer -= dt;
    ...
    m_enemies.update(dt, m_player.getPos(), m_bullets, m_fx,
        [this](sf::Vector2f pos, sf::Color col, int score) {
            if (score > 0) {
                m_comboTimer = 1.2f;
                m_combo++;
...
void Game::checkPlayerBulletVsShields() {
    for (auto& b : m_bullets.getAll()) {
        if (b.owner != BulletOwner::Player || !b.alive) continue;
...
void Game::checkEnemyBulletVsShields() {
    for (auto& b : m_bullets.getAll()) {
        if (b.owner != BulletOwner::Enemy || !b.alive) continue;
        if (m_shields.checkBulletHit(b.pos)) {
            b.alive = false;
...
void Game::render() {
    ...
    case GameState::Paused:
        ...
        drawHUD();
        if (m_state == GameState::Paused) drawPause();
        break;
    case GameState::GameOver:
        m_shields.draw(m_window);
        m_fx.draw(m_window, m_font);
...
void Game::drawPowerupHUD() {
    ...
    if (m_player.getRapidTimer() > 0.f)
        slots.push_back({ m_player.getRapidTimer(),  &GFX.powerupRapid,
                          {255, 200, 20, 255},  "RAPID" });
    if (m_player.getTripleTimer() > 0.f)
        slots.push_back({ m_player.getTripleTimer(), &GFX.powerupTriple,
                          { 80, 255, 200, 255}, "TRIPLE" });
    if (m_player.getShieldTimer() > 0.f)
        slots.push_back({ m_player.getShieldTimer(), &GFX.powerupShield,
                          {100, 150, 255, 255}, "SHIELD" });
...
void Game::drawMenu() {
...

    {
        ...
        playTxt.setPosition({ cx, btnY + btnH / 2.f - 2.f });
...

    {
        ...
        const float panelH  = headerH + BTN_H * 3.f + ROW_GAP * 2.f + 26.f;
        ...
        sf::Text ctrlTitle(m_font, "CONTROLS", 12u);
        ...
        sep.setPosition({ panelX + 10.f, panelY + 28.f });
        ...
        const float row0Y = panelY + headerH + 7.f;
        const float row1Y = row0Y + BTN_H + ROW_GAP + 2.f;
        const float row2Y = row1Y + BTN_H + ROW_GAP + 2.f;
...

        {
            ...
            shipIcon.setOrigin({ isz.x / 2.f, isz.y / 2.f });
            shipIcon.setPosition({ lineX + ICON_W / 2.f, row0Y + BTN_H / 2.f });
            m_window.draw(shipIcon);

            float x = lineX + ICON_W + COL_GAP + 10.f;
            drawKey("<", x, row0Y);  x += BTN_W + 2.f;
            drawKey(">", x, row0Y);  x += BTN_W + COL_GAP;
            drawOr(x, row0Y - 4.f);                       x += OR_W + COL_GAP;
            drawKey("A", x, row0Y);  x += BTN_W + 2.f;
            drawKey("D", x, row0Y);  x += BTN_W + COL_GAP;
            drawLabel("Move", x, row0Y);
        }

        {
            sf::Sprite bulletIcon(GFX.bulletPlayer);
            auto bsz = GFX.bulletPlayer.getSize();
            const float BULLET_ICON_SIZE = 10.f;
            ...
            float x = lineX + ICON_W + COL_GAP + 10.f;;
            drawKey("SPACE", x, row1Y, BTN_SPACE_W); x += BTN_SPACE_W + COL_GAP;
            drawOr(x, row1Y - 4.f);                         x += OR_W + COL_GAP;
            drawKey("Z", x, row1Y);                   x += BTN_W + COL_GAP;
            drawLabel("Shoot", x, row1Y);
        }

        {
            float x = lineX + ICON_W + COL_GAP + 10.f;;
            drawKey("ESC", x, row2Y, BTN_W * 1.3f); x += BTN_W * 1.3f + COL_GAP;
            drawLabel("Pause", x, row2Y);
        }
    }

    {
        ...
        hi.setPosition({ cx, float(Cfg::H) - 52.f });
        m_window.draw(hi);

        std::uint8_t alpha = static_cast<std::uint8_t>(120 + pulse * 80);
        sf::Text hint(m_font, "PRESS ENTER OR SPACE TO START", 10u);
        hint.setFillColor({ 180, 180, 180, alpha });
