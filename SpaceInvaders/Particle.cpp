...
void ParticleSystem::update(float dt) {
    ...
    m_texts.erase(
        std::remove_if(m_texts.begin(), m_texts.end(),
            [](const FloatingText& t){ return t.life <= 0; }),
        m_texts.end());
}
...
void ParticleSystem::draw(sf::RenderTarget& rt, const sf::Font& font) const {
    if (!m_particles.empty()) {
        ...
        for (std::size_t i = 0; i < m_particles.size(); ++i) {
            ...
            m_verts[vi + 0] = { tl,p.color }; m_verts[vi + 1] = { tr,p.color };
            m_verts[vi + 2] = { br,p.color }; m_verts[vi + 3] = { tl,p.color };
            m_verts[vi + 4] = { br,p.color }; m_verts[vi + 5] = { bl,p.color };
        }
        rt.draw(m_verts);
    }
