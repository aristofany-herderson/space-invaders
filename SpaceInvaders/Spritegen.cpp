                ...
                for (int dy = -radius; dy <= radius; ++dy) {
                    for (int dx = -radius; dx <= radius; ++dx) {
                        ...
                        if (nx < 0 || ny < 0 ||
                            ...
                            ny >= static_cast<int>(sz.y)) continue;
...
sf::Texture TextureBank::makePlayer() {
    ...
    for (int row = 8; row <= 28; ++row) {
        ...
        for (int dx = -half; dx <= half; ++dx) {
            sf::Color c = base;
            if (std::abs(dx) >= half - 1) c = hi;
            if (std::abs(dx) <= 1)        c = dark;
...
sf::Texture TextureBank::makeBullet(sf::Color col, float w, float h) {
    ...
    for (int y = 0; y < (int)h; ++y) {
        for (int x = 0; x < (int)w; ++x) {
            ...
            if (dist < 0.6f)       fillPixel(img, x, y, core);
            else if (dist < 1.5f)  fillPixel(img, x, y, col);
            else                    fillPixel(img, x, y, { col.r, col.g, col.b, 100 });
        }
    }
    addGlow(img, col, 2);
    return fromImage(img);
...
sf::Texture TextureBank::makeEnemy(EnemyType t) {
    ...
    switch (t) {
    case EnemyType::Drone:    base = Cfg::COL_DRONE;    hi = { 255,180,180,255 }; eye = { 255,255,100,255 }; W = 30; H = 24; break;
    case EnemyType::Invader:  base = Cfg::COL_INVADER;  hi = { 180,255,180,255 }; eye = { 255,255,100,255 }; W = 34; H = 28; break;
    case EnemyType::Brute:    base = Cfg::COL_BRUTE;    hi = { 180,200,255,255 }; eye = { 255,100,100,255 }; W = 42; H = 34; break;
    case EnemyType::Kamikaze: base = Cfg::COL_KAMIKAZE; hi = { 255,220,180,255 }; eye = { 255,80,80,255 };   W = 32; H = 30; break;
    case EnemyType::Elite:    base = Cfg::COL_ELITE;    hi = { 220,180,255,255 }; eye = { 200,255,100,255 }; W = 38; H = 32; break;
    case EnemyType::UFO:      base = Cfg::COL_UFO;      hi = { 255,240,180,255 }; eye = { 100,255,200,255 }; W = 52; H = 22; break;
    }
    sf::Image img({ (unsigned)W, (unsigned)H }, sf::Color::Transparent);
    if (t == EnemyType::UFO) {
        ...
        if (t == EnemyType::Drone) {
            ...
            fillRect(img, 0, H / 2 - 4, 7, 10, base); fillRect(img, W - 7, H / 2 - 4, 7, 10, base);
            drawCircle(img, 4, H / 2 + 1, 4, hi);        drawCircle(img, W - 5, H / 2 + 1, 4, hi);
        }
        else if (t == EnemyType::Kamikaze) {
            for (int s = 0; s < 3; ++s) fillRect(img, W / 2 - 1, H - 4 - s * 4, 3, 3, hi);
        }
        else if (t == EnemyType::Elite) {
            for (int s = -1; s <= 1; ++s) fillRect(img, W / 2 + s * 8 - 1, 0, 3, 5, base);
...
sf::Texture TextureBank::makeShieldTile(int damage) {
    ...
    sf::Color col;
    switch (damage) {
    case 0: col = { 0, 255, 100, 240 }; break;
    case 1: col = { 160, 255,  50, 200 }; break;
    case 2: col = { 255, 200,  20, 160 }; break;
    default:col = { 255,  80,  20, 100 }; break;
    }
